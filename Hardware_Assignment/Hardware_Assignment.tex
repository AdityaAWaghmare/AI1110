...
\section*{Procedure}
\begin{enumerate}
	\item First we generate the CLOCK signal using the 555 timer circuit as shown in figure \ref{555_t_c}
	\begin{figure}[h]
	...
	\item Then we connect Clock output of 555 timer circuit to the clock signal of D-Flip flops
	
	\item Now we make the circuit for shift registers using 4 D-Flip flops (using two 7474 IC's) and one XOR gate(7486 IC) as shown in figure \ref{XOR}. Pin out for 7474 IC shown in figure \ref{7474_IC} 
	\begin{figure}[h]
	...
	\item Now we connect XOR gate (7486 IC) according to the figure \ref{XOR} 
	\begin{figure}[h]
	...
	\item After that we connect the decoder (7447 IC) and connect its A,B,C,D with $D_0$,$D_1$,$D_2$,$D_3$ respectively as per the figure \ref{7447}
	\begin{figure}[h]
	...
	\item Then we place The seven segmented display and connect it with the decoder (7447 IC) according to the table \ref{table} and the figure \ref{SSD}
	\begin{figure}[h]
	...
	\item Additionally we make connections like Vcc and GND to every IC as per the respective IC pinout for IC's 7474, 7447, 7486. 
	
	\item Output was changing digits on the seven segment display the output is shown in figure \ref{output}
	\begin{figure}[h]
