...
\section*{Procedure}
\begin{enumerate}
	\item First ee connect the 555 timer circuit according to the figure \ref{555_t_c}
	\begin{figure}[h]
	...
	\item Then we connect XOR gate (7486 IC) according to the figure \ref{XOR} 
	\begin{figure}[h]
	...
	\item After that we connected the decoder (7447 IC) and connected its A,B,C,D with $D_0$,$D_1$,$D_2$,$D_3$ respectively as per the figure \ref{7447}
	\begin{figure}[h]
	...
	\item Then we place The seven segmented display and connect it with the dceoder (7447 IC) according to the table \ref{table} and the figure \ref{SSD}
	\begin{figure}[h]
	...
	\item We connect all the independent parts with each other and connect it to the power source
